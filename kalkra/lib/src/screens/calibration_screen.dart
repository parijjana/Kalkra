...
class _CalibrationScreenState extends ConsumerState<CalibrationScreen> {
  ...
  Future<void> _startGeneration() async {
    ...
    // For Multi-Target modes, generate only Round 1 immediately to reduce latency
    final bool isMultiTarget = widget.gameMode == GameMode.tripleThreat || widget.gameMode == GameMode.doubleDanger;
    final int initialGenCount = isMultiTarget ? 1 : widget.totalRounds;
    ...
    if (isMultiTarget && widget.totalRounds > 1) {
      _startBackgroundGeneration(match, seed);
    }

    if (mounted) {
      if (isMultiTarget) {
        Navigator.of(context).pushReplacement(
          MaterialPageRoute(builder: (context) => const MultiTargetScreen()),
        );
      } else {
        Navigator.of(context).pushReplacement(
  ...
  void _startBackgroundGeneration(MatchManager match, int seed) async {
    ...
    match.appendRounds(remainingRounds);
  }
