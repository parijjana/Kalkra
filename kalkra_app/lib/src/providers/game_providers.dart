...
final transportProvider = NotifierProvider<TransportNotifier, IGameTransport>(
...
final matchStatusProvider = NotifierProvider<MatchStatusNotifier, MatchStatus>(
  MatchStatusNotifier.new,
);
...
final lastResultsProvider =
    NotifierProvider<LastResultsNotifier, Map<String, dynamic>?>(
      LastResultsNotifier.new,
    );
...
final roundStartTimeProvider = NotifierProvider<RoundStartTimeNotifier, int?>(
  RoundStartTimeNotifier.new,
);
...
final roundUpdateProvider = NotifierProvider<RoundUpdateNotifier, int>(
  RoundUpdateNotifier.new,
);
...
final sessionSyncProvider = Provider<void>((ref) {
  ref.listen<AsyncValue<GameEvent>>(gameEventStreamProvider, (prev, next) {
    next.whenData((event) {
      ...
      switch (event.type) {
        // ... (playerJoined, playerLeft, playerReady, teamAssignment, teamRename, lobbyUpdate cases same)
        case GameEventType.playerJoined:
          ...
          if (isHost) {
            ...
            if (totalCount <= 6) {
              // Balance between Team 1 & 2
              int team1 = session.players.values
                  .where((p) => p.teamId == 1)
                  .length;
              int team2 = session.players.values
                  .where((p) => p.teamId == 2)
                  .length;
              session.assignTeam(player.id, team1 <= team2 ? 1 : 2);
            } else if (totalCount == 7) {
              // Full rebalance across 4 teams
              ...
              int minSize = 100;
              for (int t = 1; t <= 4; t++) {
                int size = session.players.values
                    .where((p) => p.teamId == t)
                    .length;
                if (size < minSize) {
                  minSize = size;
        ...
        case GameEventType.lobbyUpdate:
          if (!isHost) {
            ...
            final teamNames = Map<String, dynamic>.from(
              event.payload['teamNames'] ?? {},
            );
        ...
        case GameEventType.submissionReceived:
          if (isHost) {
            ...
            // Early Termination Check: If all assigned players have submitted
            if (session.allAssignedSubmitted) {
              transport.sendEvent(
                GameEvent(type: GameEventType.roundEnded, payload: {}),
              );
            }
          }
          break;
        ...
        case GameEventType.roundStarted:
          // 1. Initialize or Sync MatchManager for Clients
          if (!isHost) {
            final totalRounds = event.payload['totalRounds'] ?? 5;
            final gameModeIndex = event.payload['gameMode'];
            final gameMode = gameModeIndex != null
                ? GameMode.values[gameModeIndex]
          ...
          // 2. Unpack round data and synchronization time
        ...
        case GameEventType.roundResults:
          ...
          if (bestSolutionJson != null) {
            round.setBestSolution(
              SolveResult.fromJson(Map<String, dynamic>.from(bestSolutionJson)),
            );
          }

          // Update ELO if match is over
        ...
        case GameEventType.matchEnded:
          ...
          final totalScores = Map<String, dynamic>.from(
            event.payload['sessionTeamScores'] ?? {},
          );
          session.syncSessionHistory(
            history: history,
        ...
        case GameEventType.heartbeat:
          // Just trigger UI update to confirm connection
          break;
        case GameEventType.error:
          // Host lost or other error -> return to menu
          ref.read(matchStatusProvider.notifier).setStatus(MatchStatus.lobby);
          ...
          // Force navigation back to MainScreen
...
final isHostOnlyProvider = NotifierProvider<HostOnlyNotifier, bool>(
  HostOnlyNotifier.new,
);
...
final currentScreenIdProvider = NotifierProvider<ScreenIdNotifier, String>(
  ScreenIdNotifier.new,
);
...
final isPausedProvider = NotifierProvider<PauseNotifier, bool>(
  PauseNotifier.new,
);
...
final notificationProvider =
    NotifierProvider<NotificationNotifier, Achievement?>(
