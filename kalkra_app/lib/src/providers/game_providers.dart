...
final careerProvider = AsyncNotifierProvider<CareerNotifier, CareerManager>(CareerNotifier.new);
...
class CareerNotifier extends AsyncNotifier<CareerManager> {
  ...
  Future<void> applyEloShift(int shift, String opponentName, {bool wasSolo = false}) async {
    final manager = state.value ?? CareerManager();
  ...
  Future<void> recordSoloMatch({required int score, required String mode}) async {
    final manager = state.value ?? CareerManager();
    ...
    next.whenData((event) {
      ...
      switch (event.type) {
        ...
        case GameEventType.playerJoined:
          ...
          session.addPlayer(player.id, player.name, elo: player.currentElo, isHost: player.isHost, deviceId: player.deviceId);
          ...
          if (isHost) {
            ...
            if (totalCount <= 6) {
              ...
              // Balance across 1-4
              int bestTeam = 1; int minSize = 100;
              for (int t = 1; t <= 4; t++) {
                int size = session.players.values.where((p) => p.teamId == t).length;
                if (size < minSize) { minSize = size; bestTeam = t; }
              }
