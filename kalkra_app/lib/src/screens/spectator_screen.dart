...
class _SpectatorScreenState extends ConsumerState<SpectatorScreen>
  ...
  Future<void> _onTimeUp() async {
    ...
    if (transport is LanHostTransport) {
      ...
      if (match != null) {
        ...
        if (isMatchOver) {
          final playerElos = session.players.map(
            (id, p) => MapEntry(id, p.currentElo),
          );
          String? winnerId;
          int maxScore = -1;
  ...
  void _banPlayer(String id, String? deviceId) {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Player banned and removed.')));
  }
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<AsyncValue<GameEvent>>(gameEventStreamProvider, (prev, next) {
      next.whenData((event) {
        if (event.type == GameEventType.submissionReceived) {
          ...
          final assignedPlayers = session.players.values.where(
            (p) => p.teamId > 0,
          );
          if (assignedPlayers.isNotEmpty &&
              assignedPlayers.every((p) => p.lastExpression != null)) {
            _onTimeUp();
          }
    ...
    return Scaffold(
      ...
      appBar: isDesktop
          ? const TopNavBar(activeId: 'SpectatorScreen')
          : AppBar(
              title: Row(
                ...
                children: [
                  ...
                  Flexible(
                    child: Text(
                      'COMMAND CENTER • ROUND ${match?.currentRound ?? 1}',
                    ...
                    Row(
                      children: [
                        ...
                        Switch(
                          ...
                          onChanged: (v) => ref
                              .read(jeopardyOverrideProvider.notifier)
                              .setOverride(v),
              ...
              child: Padding(
                ...
                child: GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final teamColor = player.teamId > 0
                        ? teamColors[player.teamId - 1]
                        : Colors.grey;
