...
class _StagingScreenState extends ConsumerState<StagingScreen> {
    ...
    ref.listen<MatchStatus>(matchStatusProvider, (prev, next) {
      if (next == MatchStatus.playing) {
        ...
        Navigator.of(context).pushReplacement(
          MaterialPageRoute(
            builder: (context) =>
                isSpectating ? const SpectatorScreen() : const GameScreen(),
          ),
        );
      }
    });
          ...
          if (totalCount == 7) {
            ...
            for (final player in unassigned) {
              ...
              for (int t = 1; t <= targetTeamCount; t++) {
                final count = session.players.values
                    .where((p) => p.teamId == t)
                    .length;
                if (count < minCount) {
                  minCount = count;
                ...
                child: Column(
                  children: [
                    ...
                    if (!isDesktop && _connectionString != null)
                      _buildMobileQRHeader(colorScheme),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'Invite friends to join the arena locally.',
  ...
  void _toggleReady(bool ready) {
    ref
        .read(transportProvider)
        .sendEvent(
          GameEvent(type: GameEventType.playerReady, payload: {'ready': ready}),
        );
  }
