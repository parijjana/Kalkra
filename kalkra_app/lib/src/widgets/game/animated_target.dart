...
class AnimatedTarget extends StatelessWidget {
  ...
  const AnimatedTarget({super.key, required this.targets, required this.isHighStakes, required this.entrance, this.isDesktop = false, this.match});
  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context); final colorScheme = theme.colorScheme; final isDual = targets.length > 1;
    return FadeTransition(opacity: entrance, child: SlideTransition(position: Tween<Offset>(begin: const Offset(0, -0.2), end: Offset.zero).animate(CurvedAnimation(parent: entrance, curve: Curves.easeOutBack)), child: Container(width: double.infinity, alignment: Alignment.center, decoration: BoxDecoration(color: isHighStakes ? Colors.red.withValues(alpha: 0.1) : colorScheme.surfaceContainerLow, borderRadius: BorderRadius.vertical(bottom: Radius.circular(isDesktop ? 80 : 56)), boxShadow: [BoxShadow(color: isHighStakes ? Colors.red.withValues(alpha: 0.2) : colorScheme.onSurface.withValues(alpha: 0.05), blurRadius: 50, offset: const Offset(0, 20))]), child: Stack(children: [
      Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        if (isDual) Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              _buildTargetText(targets[0], theme, colorScheme.primary),
              const SizedBox(width: 40),
              _buildTargetText(targets[1], theme, colorScheme.secondary),
        ])) else _buildTargetText(targets.isNotEmpty ? targets.first : 0, theme, isHighStakes ? Colors.redAccent : colorScheme.primary)
      ]),
  ...
  Widget _buildTargetText(int val, ThemeData theme, Color color) => FittedBox(fit: BoxFit.scaleDown, child: Text('$val', style: theme.textTheme.displayLarge?.copyWith(color: color, fontSize: isDesktop ? 120 : 110, height: 1, fontWeight: FontWeight.w900)));
}
