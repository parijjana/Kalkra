...
class GameHeader extends ConsumerWidget implements PreferredSizeWidget {
  final String roundText; final int myScore; final int secondsLeft; final VoidCallback onExit; final VoidCallback onClear;
  const GameHeader({super.key, required this.roundText, required this.myScore, required this.secondsLeft, required this.onExit, required this.onClear});
  @override
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AppBar(
      ...
      title: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text('TIME: $secondsLeft', style: theme.textTheme.titleLarge?.copyWith(color: colorScheme.onPrimary, fontWeight: FontWeight.w900, letterSpacing: 2)),
      ]),
      centerTitle: true, elevation: 0,
      actions: [IconButton(onPressed: onExit, icon: const Icon(Icons.logout_rounded)), IconButton(onPressed: onClear, icon: const Icon(Icons.refresh_rounded))],
    );
  }
}
