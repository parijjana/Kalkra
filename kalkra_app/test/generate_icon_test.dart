...
void main() {
  test('Generate PNG icon from SVG spec with transparency', () {
    ...
    void drawRoundedQuadrant(int x, int y, int size, img.Color color, {bool tl = false, bool tr = false, bool bl = false, bool br = false}) {
      final r = 140;
      img.fillRect(image, x1: x + (tl ? r : 0), y1: y, x2: x + size - (tr ? r : 0), y2: y + size, color: color);
      img.fillRect(image, x1: x, y1: y + (tl || tr ? r : 0), x2: x + size, y2: y + size - (bl || br ? r : 0), color: color);
      
      if (tl) img.fillCircle(image, x: x + r, y: y + r, radius: r, color: color);
      if (tr) img.fillCircle(image, x: x + size - r, y: y + r, radius: r, color: color);
      if (bl) img.fillCircle(image, x: x + r, y: y + size - r, radius: r, color: color);
      if (br) img.fillCircle(image, x: x + size - r, y: y + size - r, radius: r, color: color);
    }

    // 1. Draw Exploded Quadrants
    drawRoundedQuadrant(padding, padding, quadSize, cyan, tl: true);
    drawRoundedQuadrant(padding + quadSize + gap, padding, quadSize, pink, tr: true);
    drawRoundedQuadrant(padding, padding + quadSize + gap, quadSize, purple, bl: true);
    drawRoundedQuadrant(padding + quadSize + gap, padding + quadSize + gap, quadSize, yellow, br: true);

    // 2. Draw Negative Space Symbols (Transparent Cutouts)
    // PLUS (TL)
    int tlX = padding + quadSize ~/ 2;
    int tlY = padding + quadSize ~/ 2;
    img.fillRect(image, x1: tlX - 100, y1: tlY - 15, x2: tlX + 100, y2: tlY + 15, color: transparent);
    img.fillRect(image, x1: tlX - 15, y1: tlY - 100, x2: tlX + 15, y2: tlY + 100, color: transparent);
    ...
    img.fillRect(image, x1: trX - 100, y1: trY - 15, x2: trX + 100, y2: trY + 15, color: transparent);
    ...
    for (int i = -12; i <= 12; i++) {
       img.drawLine(image, x1: blX - 80 + i, y1: blY - 80, x2: blX + 80 + i, y2: blY + 80, color: transparent);
       img.drawLine(image, x1: blX + 80 + i, y1: blY - 80, x2: blX - 80 + i, y2: blY + 80, color: transparent);
    }
    ...
    img.fillRect(image, x1: brX - 100, y1: brY - 15, x2: brX + 100, y2: brY + 15, color: transparent);
