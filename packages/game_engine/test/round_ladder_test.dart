...
void main() {
  group('MatchManager: Round Ladder', () {
    test('Progressive mode follows the canonical 8-round sequence', () {
      ...
      expect(match.currentConfig.title, equals(RoundConfig.classic.title));
      ...
      expect(match.currentConfig.title, equals(RoundConfig.gauntlet.title));

      // Round 4: Forbidden Number
      match.nextRound();
      expect(match.currentRound, 4);
      expect(match.currentConfig.title, equals(RoundConfig.forbiddenNumber.title));

      // Round 5: Two Targets
      match.nextRound();
      expect(match.currentRound, 5);
      expect(match.currentConfig.title, equals(RoundConfig.twoTargets.title));

      // Round 6: Expanding Pool
      match.nextRound();
      expect(match.currentRound, 6);
      expect(match.currentConfig.title, equals(RoundConfig.expandingPool.title));

      // Round 7: Mandatory Number
      match.nextRound();
      expect(match.currentRound, 7);
      expect(match.currentConfig.title, equals(RoundConfig.mandatoryNumber.title));
    });
  });
}
